...
def hierquery(qstring,qtype):
	"""Given a query type qtype, returns answers of that type for lookup qstring. If no answers are found, removes the most specific component(the part before the leftmost period) and retries the query with the result. If the topmost query fails, returns None."""
...
def findnameservers(hostname):
	"""Attempts to determine the authoritative nameservers for a given hostname .Return None on failure."""
